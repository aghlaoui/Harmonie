...
function harmonie_form_field($key, $args, $value = null)
{
    $defaults = array(
        'type'              => 'text',
        'label'             => '',
    ...
    $field_container = '<div class="col-md-12 space-t-15 form-row %1$s" id="%2$s" data-priority="' . esc_attr($sort) . '">%3$s</div>';
    ...
    if (!empty($field)) {
        ...
        if ($args['label'] && 'checkbox' !== $args['type']) {
            $field_html .= '<label for="' . esc_attr($label_id) . '" class="form-label ' . esc_attr(implode(' ', $args['label_class'])) . '">' . wp_kses_post($args['label']) . $required . '</label>';
        }
